...
\normalsize \hypertarget{md_node_modules_mkdirp_readme_autotoc_md770}{}\doxysection{example}\label{md_node_modules_mkdirp_readme_autotoc_md770}
\hypertarget{md_node_modules_mkdirp_readme_autotoc_md771}{}\doxysubsection{pow.\+js}\label{md_node_modules_mkdirp_readme_autotoc_md771}

\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} mkdirp = require(\textcolor{stringliteral}{'mkdirp'})}
\DoxyCodeLine{}
...
And now /tmp/foo/bar/baz exists, huzzah!\hypertarget{md_node_modules_mkdirp_readme_autotoc_md772}{}\doxysection{methods}\label{md_node_modules_mkdirp_readme_autotoc_md772}

\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} mkdirp = require(\textcolor{stringliteral}{'mkdirp'})}
\end{DoxyCode}
\hypertarget{md_node_modules_mkdirp_readme_autotoc_md773}{}\doxysubsection{mkdirp(dir, \mbox{[}opts\mbox{]}) -\/$>$ Promise$<$\+String $\vert$ undefined$>$}\label{md_node_modules_mkdirp_readme_autotoc_md773}
...
You can also override just one or the other of {\ttfamily mkdir} and {\ttfamily stat} by passing in {\ttfamily opts.\+stat} or {\ttfamily opts.\+mkdir}, or providing an {\ttfamily fs} option that only overrides one of these.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md774}{}\doxysubsection{mkdirp.\+sync(dir, opts) -\/$>$ String$\vert$null}\label{md_node_modules_mkdirp_readme_autotoc_md774}
...
You can also override just one or the other of {\ttfamily mkdir\+Sync} and {\ttfamily stat\+Sync} by passing in {\ttfamily opts.\+stat\+Sync} or {\ttfamily opts.\+mkdir\+Sync}, or providing an {\ttfamily fs} option that only overrides one of these.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md775}{}\doxysubsection{mkdirp.\+manual, mkdirp.\+manual\+Sync}\label{md_node_modules_mkdirp_readme_autotoc_md775}
Use the manual implementation (not the native one). This is the default when the native implementation is not available or the stat/mkdir implementation is overridden.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md776}{}\doxysubsection{mkdirp.\+native, mkdirp.\+native\+Sync}\label{md_node_modules_mkdirp_readme_autotoc_md776}
Use the native implementation (not the manual one). This is the default when the native implementation is available and stat/mkdir are not overridden.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md777}{}\doxysection{implementation}\label{md_node_modules_mkdirp_readme_autotoc_md777}
On Node.\+js v10.\+12.\+0 and above, use the native {\ttfamily fs.\+mkdir(p, \{recursive\+:true\})} option, unless {\ttfamily fs.\+mkdir}/{\ttfamily fs.\+mkdir\+Sync} has been overridden by an option.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md778}{}\doxysubsection{native implementation}\label{md_node_modules_mkdirp_readme_autotoc_md778}
...
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md779}{}\doxysubsection{manual implementation}\label{md_node_modules_mkdirp_readme_autotoc_md779}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md780}{}\doxysubsection{windows vs unix caveat}\label{md_node_modules_mkdirp_readme_autotoc_md780}
...
In order to work around this issue, the native implementation will fall back to the manual implementation if an {\ttfamily E\+N\+O\+E\+NT} error is encountered.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md782}{}\doxysection{choosing a recursive mkdir implementation}\label{md_node_modules_mkdirp_readme_autotoc_md782}
There are a few to choose from! Use the one that suits your needs best \+:D\hypertarget{md_node_modules_mkdirp_readme_autotoc_md783}{}\doxysubsection{use $<$tt$>$fs.\+mkdir(path, \{recursive\+: true\}, cb)$<$/tt$>$ if\+:}\label{md_node_modules_mkdirp_readme_autotoc_md783}
...
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md784}{}\doxysubsection{use this module (mkdirp 1.\+x) if\+:}\label{md_node_modules_mkdirp_readme_autotoc_md784}
...
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md785}{}\doxysubsection{use $<$a href=\char`\"{}http\+://npm.\+im/make-\/dir\char`\"{}$>$$<$tt$>$make-\/dir$<$/tt$>$$<$/a$>$ if\+:}\label{md_node_modules_mkdirp_readme_autotoc_md785}
...
\item You don\textquotesingle{}t need/want a C\+LI.
\item You need to override the {\ttfamily fs} methods in use.
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md786}{}\doxysubsection{use mkdirp 0.\+x if\+:}\label{md_node_modules_mkdirp_readme_autotoc_md786}
...
\end{DoxyItemize}\hypertarget{md_node_modules_mkdirp_readme_autotoc_md787}{}\doxysection{cli}\label{md_node_modules_mkdirp_readme_autotoc_md787}
...
\end{DoxyCode}
\hypertarget{md_node_modules_mkdirp_readme_autotoc_md788}{}\doxysection{install}\label{md_node_modules_mkdirp_readme_autotoc_md788}
...
to run the command without installing it globally.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md789}{}\doxysection{platform support}\label{md_node_modules_mkdirp_readme_autotoc_md789}
This module works on node v8, but only v10 and above are officially supported, as \mbox{\hyperlink{classNode}{Node}} v8 reached its L\+TS end of life 2020-\/01-\/01, which is in the past, as of this writing.\hypertarget{md_node_modules_mkdirp_readme_autotoc_md790}{}\doxysection{license}\label{md_node_modules_mkdirp_readme_autotoc_md790}
M\+IT 