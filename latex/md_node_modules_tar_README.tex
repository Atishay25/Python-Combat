...
The A\+PI is designed to mimic the behavior of {\ttfamily tar(1)} on unix systems. If you are familiar with how tar works, most of this will hopefully be straightforward for you. If not, then hopefully this module can teach you useful unix skills that may come in handy someday \+:)\hypertarget{md_node_modules_tar_README_autotoc_md1638}{}\doxysection{Background}\label{md_node_modules_tar_README_autotoc_md1638}
...
The other flags and options modify how this top level function works.\hypertarget{md_node_modules_tar_README_autotoc_md1639}{}\doxysection{High-\/\+Level A\+PI}\label{md_node_modules_tar_README_autotoc_md1639}
These 5 functions are the high-\/level A\+PI. All of them have a single-\/character name (for unix nerds familiar with {\ttfamily tar(1)}) as well as a long name (for everyone else).
...
Sync commands without a file argument return a stream that acts on its input immediately in the same tick. For readable streams, this means that all of the data is immediately available by calling {\ttfamily stream.\+read()}. For writable streams, it will be acted upon as soon as it is provided, but this can be at any time.\hypertarget{md_node_modules_tar_README_autotoc_md1640}{}\doxysubsection{Warnings and Errors}\label{md_node_modules_tar_README_autotoc_md1640}
...
\item {\ttfamily entry} The entry object (if it could be created) for {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+FO}, {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+V\+A\+L\+ID}, and {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+E\+R\+R\+OR} warnings.
\item {\ttfamily header} The header object (if it could be created, and the entry could not be created) for {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+FO} and {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+V\+A\+L\+ID} warnings.
\item {\ttfamily recoverable} Boolean. If {\ttfamily false}, then the warning will emit an {\ttfamily error}, even in non-\/strict mode.
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1641}{}\doxysubsubsection{Error Codes}\label{md_node_modules_tar_README_autotoc_md1641}

\begin{DoxyItemize}
\item {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+FO} An informative error indicating that an entry is being modified, but otherwise processed normally. For example, removing {\ttfamily /} or {\ttfamily C\+:\textbackslash{}} from absolute paths if {\ttfamily preserve\+Paths} is not set.
\item {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+V\+A\+L\+ID} An indication that a given entry is not a valid tar archive entry, and will be skipped. This occurs when\+:
\begin{DoxyItemize}
...
If every entry in a parsed archive raises an {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+I\+N\+V\+A\+L\+ID} error, then the archive is presumed to be unrecoverably broken, and {\ttfamily T\+A\+R\+\_\+\+B\+A\+D\+\_\+\+A\+R\+C\+H\+I\+VE} will be raised.
\item {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+E\+R\+R\+OR} The entry appears to be a valid tar archive entry, but encountered an error which prevented it from being unpacked. This occurs when\+:
\begin{DoxyItemize}
...
\end{DoxyItemize}
\item {\ttfamily T\+A\+R\+\_\+\+E\+N\+T\+R\+Y\+\_\+\+U\+N\+S\+U\+P\+P\+O\+R\+T\+ED} An indication that a given entry is a valid archive entry, but of a type that is unsupported, and so will be skipped in archive creation or extracting.
\item {\ttfamily T\+A\+R\+\_\+\+A\+B\+O\+RT} When parsing gzipped-\/encoded archives, the parser will abort the parse process raise a warning for any zlib errors encountered. Aborts are considered unrecoverable for both parsing and unpacking.
\item {\ttfamily T\+A\+R\+\_\+\+B\+A\+D\+\_\+\+A\+R\+C\+H\+I\+VE} The archive file is totally hosed. This can happen for a number of reasons, and always occurs at the end of a parse or extract\+:
\begin{DoxyItemize}
...
{\ttfamily T\+A\+R\+\_\+\+B\+A\+D\+\_\+\+A\+R\+C\+H\+I\+VE} is considered informative for parse operations, but unrecoverable for extraction. Note that, if encountered at the end of an extraction, tar W\+I\+LL still have extracted as much it could from the archive, so there may be some garbage files to clean up.
\end{DoxyItemize}
...
Errors generated by tar will have one of the above codes set as the {\ttfamily error.\+code} field as well, but since errors originating in zlib or fs will have their original codes, it\textquotesingle{}s better to read {\ttfamily error.\+tar\+Code} if you wish to see how tar is handling the issue.\hypertarget{md_node_modules_tar_README_autotoc_md1642}{}\doxysubsection{Examples}\label{md_node_modules_tar_README_autotoc_md1642}
The A\+PI mimics the {\ttfamily tar(1)} command line functionality, with aliases for more human-\/readable option and function names. The goal is that if you know how to use {\ttfamily tar(1)} in Unix, then you know how to use `require(\textquotesingle{}tar')\`{} in Java\+Script.
...
\DoxyCodeLine{tar.c(}
\DoxyCodeLine{  \{}
\DoxyCodeLine{    gzip: <\textcolor{keyword}{true}|gzip \mbox{\hyperlink{bin_2markdown-it_8js_a14d7068fe04a78458c0b19f26a4f10d8}{options}}>,}
\DoxyCodeLine{    file: \textcolor{stringliteral}{'my-\/tarball.tgz'}}
\DoxyCodeLine{  \},}
\DoxyCodeLine{  [\textcolor{stringliteral}{'some'}, \textcolor{stringliteral}{'files'}, \textcolor{stringliteral}{'and'}, \textcolor{stringliteral}{'folders'}]}
\DoxyCodeLine{).then(\mbox{\hyperlink{salty_8js_aea07e28bf8bb82799d8434d63dd4be73}{\_}} => \{ .. tarball \mbox{\hyperlink{has_2test_2index_8js_a459ad85053f69363b5b9283fdeb47760}{has}} been created .. \})}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{tar.c( \textcolor{comment}{// or tar.create}}
\DoxyCodeLine{  \{}
\DoxyCodeLine{    gzip: <\textcolor{keyword}{true}|gzip \mbox{\hyperlink{bin_2markdown-it_8js_a14d7068fe04a78458c0b19f26a4f10d8}{options}}>}
\DoxyCodeLine{  \},}
\DoxyCodeLine{  [\textcolor{stringliteral}{'some'}, \textcolor{stringliteral}{'files'}, \textcolor{stringliteral}{'and'}, \textcolor{stringliteral}{'folders'}]}
\DoxyCodeLine{).pipe(\mbox{\hyperlink{babel-parser_8js_a3569369cd6a05edf32de460fd4a6bddf}{fs}}.createWriteStream(\textcolor{stringliteral}{'my-\/tarball.tgz'}))}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{tar.x(  \textcolor{comment}{// or tar.extract(}}
\DoxyCodeLine{  \{}
\DoxyCodeLine{    file: \textcolor{stringliteral}{'my-\/tarball.tgz'}}
\DoxyCodeLine{  \}}
\DoxyCodeLine{).then(\mbox{\hyperlink{salty_8js_aea07e28bf8bb82799d8434d63dd4be73}{\_}}=> \{ .. tarball \mbox{\hyperlink{has_2test_2index_8js_a459ad85053f69363b5b9283fdeb47760}{has}} been dumped in cwd .. \})}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{babel-parser_8js_a3569369cd6a05edf32de460fd4a6bddf}{fs}}.createReadStream(\textcolor{stringliteral}{'my-\/tarball.tgz'}).pipe(}
\DoxyCodeLine{  tar.x(\{}
\DoxyCodeLine{    strip: 1,}
\DoxyCodeLine{    C: \textcolor{stringliteral}{'some-\/dir'} \textcolor{comment}{// alias for cwd:'some-\/dir', also ok}}
\DoxyCodeLine{  \})}
\DoxyCodeLine{)}
\end{DoxyCode}
...
\DoxyCodeLine{tar.t(\{}
\DoxyCodeLine{  file: \textcolor{stringliteral}{'my-\/tarball.tgz'},}
\DoxyCodeLine{  onentry: entry => \{ .. \textcolor{keywordflow}{do} whatever with \mbox{\hyperlink{lodash_8min_8js_a53bb00c7767c3df3cdef39d2863ce11f}{it}} .. \}}
\DoxyCodeLine{\})}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} getEntryFilenames = \mbox{\hyperlink{resolve_2index_8js_a8a58c0ef8ca44edf99e5325df2dcf083}{async}} tarballFilename => \{}
\DoxyCodeLine{  \textcolor{keyword}{const} filenames = []}
\DoxyCodeLine{  await tar.t(\{}
\DoxyCodeLine{    file: tarballFilename,}
\DoxyCodeLine{    onentry: entry => filenames.push(entry.path),}
\DoxyCodeLine{  \})}
\DoxyCodeLine{  \textcolor{keywordflow}{return} filenames}
\DoxyCodeLine{\}}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{babel-parser_8js_a3569369cd6a05edf32de460fd4a6bddf}{fs}}.createReadStream(\textcolor{stringliteral}{'my-\/tarball.tgz'})}
\DoxyCodeLine{  .pipe(tar.t())}
\DoxyCodeLine{  .on(\textcolor{stringliteral}{'entry'}, entry => \{ .. \textcolor{keywordflow}{do} whatever with \mbox{\hyperlink{lodash_8min_8js_a53bb00c7767c3df3cdef39d2863ce11f}{it}} .. \})}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} getEntryFilenamesSync = tarballFilename => \{}
\DoxyCodeLine{  \textcolor{keyword}{const} filenames = []}
\DoxyCodeLine{  tar.t(\{}
\DoxyCodeLine{    file: tarballFilename,}
\DoxyCodeLine{    onentry: entry => filenames.push(entry.path),}
\DoxyCodeLine{    \mbox{\hyperlink{resolve_2index_8js_a4ea10e180af110ac020249ec7cb36086}{sync}}: \textcolor{keyword}{true},}
\DoxyCodeLine{  \})}
\DoxyCodeLine{  \textcolor{keywordflow}{return} filenames}
\DoxyCodeLine{\}}
\end{DoxyCode}
...
Keep reading for all the command descriptions and options, as well as the low-\/level A\+PI that they are built on.\hypertarget{md_node_modules_tar_README_autotoc_md1643}{}\doxysubsection{tar.\+c(options, file\+List, callback) \mbox{[}alias\+: tar.\+create\mbox{]}}\label{md_node_modules_tar_README_autotoc_md1643}
...
\item {\ttfamily cwd} The current working directory for creating the archive. Defaults to {\ttfamily process.\+cwd()}. \mbox{[}Alias\+: {\ttfamily C}\mbox{]}
...
\item {\ttfamily portable} Omit metadata that is system-\/specific\+: {\ttfamily ctime}, {\ttfamily atime}, {\ttfamily uid}, {\ttfamily gid}, {\ttfamily uname}, {\ttfamily gname}, {\ttfamily dev}, {\ttfamily ino}, and {\ttfamily nlink}. Note that {\ttfamily mtime} is still included, because this is necessary for other time-\/based operations. Additionally, {\ttfamily mode} is set to a \char`\"{}reasonable
  default\char`\"{} for most unix systems, based on a {\ttfamily umask} value of {\ttfamily 0o22}.
...
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1644}{}\doxysubsection{tar.\+x(options, file\+List, callback) \mbox{[}alias\+: tar.\+extract\mbox{]}}\label{md_node_modules_tar_README_autotoc_md1644}
...
\begin{DoxyItemize}
\item {\ttfamily max\+Read\+Size} The maximum buffer size for {\ttfamily fs.\+read()} operations. Defaults to 16 MB.
\item {\ttfamily umask} \mbox{\hyperlink{classFilter}{Filter}} the modes of entries like {\ttfamily process.\+umask()}.
...
Note that using an asynchronous stream type with the {\ttfamily transform} option will cause undefined behavior in sync extractions. \href{http://npm.im/minipass}{\texttt{ Mini\+Pass}}-\/based streams are designed for this use case.\hypertarget{md_node_modules_tar_README_autotoc_md1645}{}\doxysubsection{tar.\+t(options, file\+List, callback) \mbox{[}alias\+: tar.\+list\mbox{]}}\label{md_node_modules_tar_README_autotoc_md1645}
...
\item {\ttfamily onwarn} A function that will get called with {\ttfamily (code, message, data)} for any warnings encountered. (See \char`\"{}\+Warnings and Errors\char`\"{})
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1646}{}\doxysubsection{tar.\+u(options, file\+List, callback) \mbox{[}alias\+: tar.\+update\mbox{]}}\label{md_node_modules_tar_README_autotoc_md1646}
...
\item {\ttfamily no\+Mtime} Set to true to omit writing {\ttfamily mtime} values for entries. Note that this prevents using other mtime-\/based features like {\ttfamily tar.\+update} or the {\ttfamily keep\+Newer} option with the resulting tar archive. \mbox{[}Alias\+: {\ttfamily m}, {\ttfamily no-\/mtime}\mbox{]}
\item {\ttfamily mtime} Set to a {\ttfamily Date} object to force a specific {\ttfamily mtime} for everything added to the archive. Overridden by {\ttfamily no\+Mtime}.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1647}{}\doxysubsection{tar.\+r(options, file\+List, callback) \mbox{[}alias\+: tar.\+replace\mbox{]}}\label{md_node_modules_tar_README_autotoc_md1647}
...
\item {\ttfamily no\+Mtime} Set to true to omit writing {\ttfamily mtime} values for entries. Note that this prevents using other mtime-\/based features like {\ttfamily tar.\+update} or the {\ttfamily keep\+Newer} option with the resulting tar archive. \mbox{[}Alias\+: {\ttfamily m}, {\ttfamily no-\/mtime}\mbox{]}
\item {\ttfamily mtime} Set to a {\ttfamily Date} object to force a specific {\ttfamily mtime} for everything added to the archive. Overridden by {\ttfamily no\+Mtime}.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1648}{}\doxysection{Low-\/\+Level A\+PI}\label{md_node_modules_tar_README_autotoc_md1648}
\hypertarget{md_node_modules_tar_README_autotoc_md1649}{}\doxysubsection{class tar.\+Pack}\label{md_node_modules_tar_README_autotoc_md1649}
A readable tar stream.

Has all the standard readable stream interface stuff. `\textquotesingle{}data'{\ttfamily and }\textquotesingle{}end\textquotesingle{}{\ttfamily events,}read(){\ttfamily method,}pause(){\ttfamily and}resume()\`{}, etc.\hypertarget{md_node_modules_tar_README_autotoc_md1650}{}\doxysubsubsection{constructor(options)}\label{md_node_modules_tar_README_autotoc_md1650}
The following options are supported\+:
...
\item {\ttfamily mtime} Set to a {\ttfamily Date} object to force a specific {\ttfamily mtime} for everything added to the archive. Overridden by {\ttfamily no\+Mtime}.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1651}{}\doxysubsubsection{add(path)}\label{md_node_modules_tar_README_autotoc_md1651}
Adds an entry to the archive. Returns the Pack stream.\hypertarget{md_node_modules_tar_README_autotoc_md1652}{}\doxysubsubsection{write(path)}\label{md_node_modules_tar_README_autotoc_md1652}
Adds an entry to the archive. Returns true if flushed.\hypertarget{md_node_modules_tar_README_autotoc_md1653}{}\doxysubsubsection{end()}\label{md_node_modules_tar_README_autotoc_md1653}
Finishes the archive.\hypertarget{md_node_modules_tar_README_autotoc_md1654}{}\doxysubsection{class tar.\+Pack.\+Sync}\label{md_node_modules_tar_README_autotoc_md1654}
Synchronous version of {\ttfamily tar.\+Pack}.\hypertarget{md_node_modules_tar_README_autotoc_md1655}{}\doxysubsection{class tar.\+Unpack}\label{md_node_modules_tar_README_autotoc_md1655}
...
Most unpack errors will cause a {\ttfamily warn} event to be emitted. If the {\ttfamily cwd} is missing, or not a directory, then an error will be emitted.\hypertarget{md_node_modules_tar_README_autotoc_md1656}{}\doxysubsubsection{constructor(options)}\label{md_node_modules_tar_README_autotoc_md1656}
...
\item {\ttfamily onwarn} A function that will get called with {\ttfamily (code, message, data)} for any warnings encountered. (See \char`\"{}\+Warnings and Errors\char`\"{})
\item {\ttfamily no\+Chmod} Set to true to omit calling {\ttfamily fs.\+chmod()} to ensure that the extracted file matches the entry mode. This also suppresses the call to {\ttfamily process.\+umask()} to determine the default umask value, since tar will extract with whatever mode is provided, and let the process {\ttfamily umask} apply normally.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1657}{}\doxysubsection{class tar.\+Unpack.\+Sync}\label{md_node_modules_tar_README_autotoc_md1657}
Synchronous version of {\ttfamily tar.\+Unpack}.

Note that using an asynchronous stream type with the {\ttfamily transform} option will cause undefined behavior in sync unpack streams. \href{http://npm.im/minipass}{\texttt{ Mini\+Pass}}-\/based streams are designed for this use case.\hypertarget{md_node_modules_tar_README_autotoc_md1658}{}\doxysubsection{class tar.\+Parse}\label{md_node_modules_tar_README_autotoc_md1658}
...
Each {\ttfamily entry} will not emit until the one before it is flushed through, so make sure to either consume the data (with `on(\textquotesingle{}data', ...){\ttfamily or }.pipe(...){\ttfamily ) or throw it away with}.resume()\`{} to keep the stream flowing.\hypertarget{md_node_modules_tar_README_autotoc_md1659}{}\doxysubsubsection{constructor(options)}\label{md_node_modules_tar_README_autotoc_md1659}
...
\item {\ttfamily onentry} A function that gets called with {\ttfamily (entry)} for each entry that passes the filter.
\item {\ttfamily onwarn} A function that will get called with {\ttfamily (code, message, data)} for any warnings encountered. (See \char`\"{}\+Warnings and Errors\char`\"{})
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1660}{}\doxysubsubsection{abort(error)}\label{md_node_modules_tar_README_autotoc_md1660}
Stop all parsing activities. This is called when there are zlib errors. It also emits an unrecoverable warning with the error provided.\hypertarget{md_node_modules_tar_README_autotoc_md1661}{}\doxysubsection{class tar.\+Read\+Entry extends $<$a href=\char`\"{}http\+://npm.\+im/minipass\char`\"{}$>$\+Mini\+Pass$<$/a$>$}\label{md_node_modules_tar_README_autotoc_md1661}
...
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1662}{}\doxysubsubsection{constructor(header, extended, global\+Extended)}\label{md_node_modules_tar_README_autotoc_md1662}
Create a new Read\+Entry object with the specified header, extended header, and global extended header values.\hypertarget{md_node_modules_tar_README_autotoc_md1663}{}\doxysubsection{class tar.\+Write\+Entry extends $<$a href=\char`\"{}http\+://npm.\+im/minipass\char`\"{}$>$\+Mini\+Pass$<$/a$>$}\label{md_node_modules_tar_README_autotoc_md1663}
...
\item {\ttfamily myuser} The {\ttfamily env.\+U\+S\+ER} string if set, or `'\textquotesingle{}{\ttfamily . Set as the entry }uname{\ttfamily field if the file\textquotesingle{}s}uid{\ttfamily matches}this.\+myuid$<$tt$>$. -\/max\+Read\+Size{\ttfamily The maximum buffer size for}fs.\+read()\`{} operations. Defaults to 1 MB.
\item {\ttfamily link\+Cache} A Map object containing the device and inode value for any file whose nlink is $>$ 1, to identify hard links.
\item {\ttfamily stat\+Cache} A Map object that caches calls {\ttfamily lstat}.
\item {\ttfamily preserve\+Paths} Allow absolute paths. By default, {\ttfamily /} is stripped from absolute paths.
\item {\ttfamily cwd} The current working directory for creating the archive. Defaults to {\ttfamily process.\+cwd()}.
...
\item {\ttfamily no\+Pax} Suppress pax extended headers. Note that this means that long paths and linkpaths will be truncated, and large or negative numeric values may be interpreted incorrectly.
\item {\ttfamily no\+Mtime} Set to true to omit writing {\ttfamily mtime} values for entries. Note that this prevents using other mtime-\/based features like {\ttfamily tar.\+update} or the {\ttfamily keep\+Newer} option with the resulting tar archive.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1664}{}\doxysubsubsection{constructor(path, options)}\label{md_node_modules_tar_README_autotoc_md1664}
...
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1665}{}\doxysubsubsection{warn(message, data)}\label{md_node_modules_tar_README_autotoc_md1665}
If strict, emit an error with the provided message.

Othewise, emit a `\textquotesingle{}warn'\`{} event with the provided message and data.\hypertarget{md_node_modules_tar_README_autotoc_md1666}{}\doxysubsection{class tar.\+Write\+Entry.\+Sync}\label{md_node_modules_tar_README_autotoc_md1666}
Synchronous version of tar.\+Write\+Entry\hypertarget{md_node_modules_tar_README_autotoc_md1667}{}\doxysubsection{class tar.\+Write\+Entry.\+Tar}\label{md_node_modules_tar_README_autotoc_md1667}
A version of tar.\+Write\+Entry that gets its data from a tar.\+Read\+Entry instead of from the filesystem.\hypertarget{md_node_modules_tar_README_autotoc_md1668}{}\doxysubsubsection{constructor(read\+Entry, options)}\label{md_node_modules_tar_README_autotoc_md1668}
...
\item {\ttfamily onwarn} A function that will get called with {\ttfamily (code, message, data)} for any warnings encountered. (See \char`\"{}\+Warnings and Errors\char`\"{})
\item {\ttfamily no\+Mtime} Set to true to omit writing {\ttfamily mtime} values for entries. Note that this prevents using other mtime-\/based features like {\ttfamily tar.\+update} or the {\ttfamily keep\+Newer} option with the resulting tar archive.
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1669}{}\doxysubsection{class tar.\+Header}\label{md_node_modules_tar_README_autotoc_md1669}
...
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1670}{}\doxysubsubsection{constructor(data, \mbox{[}offset=0\mbox{]})}\label{md_node_modules_tar_README_autotoc_md1670}
{\ttfamily data} is optional. It is either a Buffer that should be interpreted as a tar Header starting at the specified offset and continuing for 512 bytes, or a data object of keys and values to set on the header object, and eventually encode as a tar Header.\hypertarget{md_node_modules_tar_README_autotoc_md1671}{}\doxysubsubsection{decode(block, offset)}\label{md_node_modules_tar_README_autotoc_md1671}
Decode the provided buffer starting at the specified offset.

Buffer length must be greater than 512 bytes.\hypertarget{md_node_modules_tar_README_autotoc_md1672}{}\doxysubsubsection{set(data)}\label{md_node_modules_tar_README_autotoc_md1672}
Set the fields in the data object.\hypertarget{md_node_modules_tar_README_autotoc_md1673}{}\doxysubsubsection{encode(buffer, offset)}\label{md_node_modules_tar_README_autotoc_md1673}
Encode the header fields into the buffer at the specified offset.

Returns {\ttfamily this.\+need\+Pax} to indicate whether a Pax Extended Header is required to properly encode the specified data.\hypertarget{md_node_modules_tar_README_autotoc_md1674}{}\doxysubsection{class tar.\+Pax}\label{md_node_modules_tar_README_autotoc_md1674}
...
\item {\ttfamily ino}
\item {\ttfamily nlink}
\end{DoxyItemize}\hypertarget{md_node_modules_tar_README_autotoc_md1675}{}\doxysubsubsection{constructor(object, global)}\label{md_node_modules_tar_README_autotoc_md1675}
Set the fields set in the object. {\ttfamily global} is a boolean that defaults to false.\hypertarget{md_node_modules_tar_README_autotoc_md1676}{}\doxysubsubsection{encode()}\label{md_node_modules_tar_README_autotoc_md1676}
Return a Buffer containing the header and body for the Pax extended header entry, or {\ttfamily null} if there is nothing to encode.\hypertarget{md_node_modules_tar_README_autotoc_md1677}{}\doxysubsubsection{encode\+Body()}\label{md_node_modules_tar_README_autotoc_md1677}
Return a string representing the body of the pax extended header entry.\hypertarget{md_node_modules_tar_README_autotoc_md1678}{}\doxysubsubsection{encode\+Field(field\+Name)}\label{md_node_modules_tar_README_autotoc_md1678}
Return a string representing the key/value encoding for the specified field\+Name, or `'\textquotesingle{}\`{} if the field is unset.\hypertarget{md_node_modules_tar_README_autotoc_md1679}{}\doxysubsection{tar.\+Pax.\+parse(string, extended, global)}\label{md_node_modules_tar_README_autotoc_md1679}
Return a new Pax object created by parsing the contents of the string provided.

If the {\ttfamily extended} object is set, then also add the fields from that object. (This is necessary because multiple metadata entries can occur in sequence.)\hypertarget{md_node_modules_tar_README_autotoc_md1680}{}\doxysubsection{tar.\+types}\label{md_node_modules_tar_README_autotoc_md1680}
A translation table for the {\ttfamily type} field in tar headers.\hypertarget{md_node_modules_tar_README_autotoc_md1681}{}\doxysubsubsection{tar.\+types.\+name.\+get(code)}\label{md_node_modules_tar_README_autotoc_md1681}
Get the human-\/readable name for a given alphanumeric code.\hypertarget{md_node_modules_tar_README_autotoc_md1682}{}\doxysubsubsection{tar.\+types.\+code.\+get(name)}\label{md_node_modules_tar_README_autotoc_md1682}
