...
English $\vert$ 中文 (v7.0.1)\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md728}{}\doxysection{Overview}\label{md_node_modules_markdown-it-anchor_README_autotoc_md728}
...
\end{DoxyItemize}\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md729}{}\doxysection{Usage}\label{md_node_modules_markdown-it-anchor_README_autotoc_md729}

\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} md = require(\textcolor{stringliteral}{'markdown-\/it'})()}
\DoxyCodeLine{  .use(require(\textcolor{stringliteral}{'markdown-\/it-\/anchor'}), opts)}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} \textcolor{keywordtype}{string} = require(\textcolor{stringliteral}{'string'})}
\DoxyCodeLine{const slugify = s => \textcolor{keywordtype}{string}(s).slugify().toString()}
\DoxyCodeLine{}
\DoxyCodeLine{const md = require('markdown-\/it')()}
\DoxyCodeLine{  .use(require('markdown-\/it-\/anchor'), \{ slugify \})}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} slugify = require(\textcolor{stringliteral}{'@sindresorhus/slugify'})}
\DoxyCodeLine{}
\DoxyCodeLine{const md = require('markdown-\/it')()}
\DoxyCodeLine{  .use(require('markdown-\/it-\/anchor'), \{ slugify: s => slugify(s) \})}
\end{DoxyCode}
...
\DoxyCodeLine{    .filter(token => ![\textcolor{stringliteral}{'html\_inline'}, \textcolor{stringliteral}{'image'}].includes(token.type))}
\DoxyCodeLine{    .map(t => t.content)}
\DoxyCodeLine{    .join(\textcolor{stringliteral}{''})}
\DoxyCodeLine{\}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{const} md = require(\textcolor{stringliteral}{'markdown-\/it'})()}
\DoxyCodeLine{  .use(require(\textcolor{stringliteral}{'markdown-\/it-\/anchor'}), \{ getTokensText \})}
\end{DoxyCode}
...
The anchor link will reuse the {\ttfamily id} that you explicitly defined.\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md732}{}\doxysection{Compatible table of contents plugin}\label{md_node_modules_markdown-it-anchor_README_autotoc_md732}
Looking for an automatic table of contents (T\+OC) generator? Take a look at \href{https://www.npmjs.com/package/markdown-it-toc-done-right}{\texttt{ markdown-\/it-\/toc-\/done-\/right}} it\textquotesingle{}s made from the ground to be a great companion of this plugin.\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md733}{}\doxysection{Parsing headings from H\+T\+M\+L blocks}\label{md_node_modules_markdown-it-anchor_README_autotoc_md733}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} md = require(\textcolor{stringliteral}{'markdown-\/it'})()}
\DoxyCodeLine{  .use(require(\textcolor{stringliteral}{'@npmcorp/marky-\/markdown/lib/plugin/html-\/heading'}))}
\DoxyCodeLine{  .use(require(\textcolor{stringliteral}{'markdown-\/it-\/anchor'}), opts)}
\end{DoxyCode}
...
If you need a bulletproof implementation, I would recommend the first H\+T\+ML parser approach I documented instead.\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md734}{}\doxysection{Browser example}\label{md_node_modules_markdown-it-anchor_README_autotoc_md734}
See \href{example.html}{\texttt{ {\ttfamily example.\+html}}}.\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md735}{}\doxysection{Permalinks}\label{md_node_modules_markdown-it-anchor_README_autotoc_md735}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const} anchor = require(\textcolor{stringliteral}{'markdown-\/it-\/anchor'})}
\DoxyCodeLine{const md = require('markdown-\/it')()}
\DoxyCodeLine{}
\DoxyCodeLine{md.use(anchor, \{}
\DoxyCodeLine{  permalink: anchor.permalink[styleOfPermalink](permalinkOpts)}
\DoxyCodeLine{\})}
\end{DoxyCode}
...
For the {\ttfamily symbol}, you may want to use the \href{http://graphemica.com/🔗}{\texttt{ link symbol}}, or a symbol from your favorite web font.\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md736}{}\doxysubsection{Header link}\label{md_node_modules_markdown-it-anchor_README_autotoc_md736}
This style wraps the header itself in an anchor link. It doesn\textquotesingle{}t use the {\ttfamily symbol} option as there\textquotesingle{}s no symbol needed in the markup (though you could add it with C\+SS using {\ttfamily \+::before} if you like).
...
\DoxyCodeLine{}
\DoxyCodeLine{md.use(anchor, \{}
\DoxyCodeLine{  permalink: anchor.permalink.headerLink()}
\DoxyCodeLine{\})}
\end{DoxyCode}
...
\DoxyCodeLine{}
\DoxyCodeLine{md.use(anchor, \{}
\DoxyCodeLine{  permalink: anchor.permalink.headerLink(\{ safariReaderFix: \textcolor{keyword}{true} \})}
\DoxyCodeLine{\})}
\end{DoxyCode}
...
\end{DoxyCode}
\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md737}{}\doxysubsection{Link after header}\label{md_node_modules_markdown-it-anchor_README_autotoc_md737}
...
\DoxyCodeLine{}
\DoxyCodeLine{md.use(anchor, \{}
\DoxyCodeLine{  permalink: anchor.permalink.linkAfterHeader(\{}
\DoxyCodeLine{    style: \textcolor{stringliteral}{'visually-\/hidden'},}
\DoxyCodeLine{    assistiveText: title => `Permalink to “\$\{title\}”`,}
...
$<$/details$>$\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md738}{}\doxysubsection{Link inside header}\label{md_node_modules_markdown-it-anchor_README_autotoc_md738}
...
heading, jump to heading, jump to heading\char`\"{} for each of your permalinks).\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md739}{}\doxysubsection{A\+R\+I\+A hidden}\label{md_node_modules_markdown-it-anchor_README_autotoc_md739}
...
\DoxyCodeLine{}
\DoxyCodeLine{md.use(anchor, \{}
\DoxyCodeLine{  permalink: anchor.permalink.ariaHidden(\{}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{function} renderPermalink (slug, opts, state, idx) \{\}}
\end{DoxyCode}
...
\end{DoxyCode}
\hypertarget{md_node_modules_markdown-it-anchor_README_autotoc_md742}{}\doxysection{Development}\label{md_node_modules_markdown-it-anchor_README_autotoc_md742}
